 ...
 * @returns {Promise} Promise with data or error
 */
function insert(transaction_uid, clip_uid, action, date, user_uid, host_uid, app_uid, description) {
  return db.runQuery(`INSERT INTO media_archive.transactions
        (transaction_uid, clip_uid, action, date, user_uid, host_uid, app_uid, description)
        VALUES(?, ?, ?, ?, ?, ?, ?, ?);`, [
        transaction_uid, clip_uid, action, date, user_uid, host_uid, app_uid, description]);
}
